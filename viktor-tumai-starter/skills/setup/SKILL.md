...
- Open one line yourself and confirm: exactly `model`, `input`, `tools` — **no `output`,
  no `usage`**. ~9 real model ids (`claude-opus/-sonnet/-fable-*`, `gpt-5.6-*`) with the
  official price sheet in `scripts/cost_model.py`. Some entities appear as placeholders
  (`<PERSON_ROBERT>`, `<PROJECT_NAME>`); images are placeholder data URLs.
