...
model using first-request-only task features, then selects the cheapest whose
predicted outcome is within ``QUALITY_TOLERANCE`` of the logged-model prediction
and above ``MIN_PREDICTED_OUTCOME``.  One selected model serves every call, so
the route respects the dataset premise and never creates a mid-task cache reset.

Usage: python scripts/learned_router.py export/ [results/routes.jsonl]

This is an observational prediction model, not proof of causal model quality.
offpolicy_eval.py supplies the separate matching check and its bias warnings.
"""
import json
import sys
...
def fit_tree(rows, feature_names, depth=0):
    ...
    for feature in feature_names:
        ...
        for i in range(MIN_LEAF, len(ordered) - MIN_LEAF + 1):
            ...
            if lo == hi:
                continue
            error = sse([r["outcome"] for r in left]) + sse([r["outcome"] for r in right])
...
def route_trajectory(calls, tree, models, model_counts, pricing):
    ...
    # A candidate must have at least two observed trajectories.  This is enough
    # for the small synthetic smoke test; real-export results should report and
    # scrutinize any route to such a sparse model through offpolicy support.
    supported = [m for m in models if model_counts[m] >= MIN_MODEL_SUPPORT]
    viable = []
...
# - The tree learns correlations in the logged policy, not experimental causal effects.
#   Harder tasks may preferentially have been given a stronger model.
# - Structural outcome is not correctness and the final response is unobserved.
# - Candidate-model one-hot flags need overlap: a sparsely logged model is never selected.
# - Only input-side cache-aware cost is estimable; output tokens/cost are missing.
...
def main():
    ...
    models = sorted({calls[0]["model"] for calls in groups.values()
                     if calls and len({c["model"] for c in calls}) == 1})
    ...
    with open(destination, "w") as out:
        for key, calls in groups.items():
            ...
            total_logged += cost_logged; total_routed += cost_routed; decisions[reason] += 1
